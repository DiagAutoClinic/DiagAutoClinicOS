...
Version: 3.2.0 - Glassmorphic Design with Neon Clinic Theme
"""
...
from shared.animated_bg import NeonClinicBG
...
class ModernLauncher(QMainWindow):
    ...
    def create_hero_section(self):
        ...
        theme_label.setProperty("class", "theme-label")
    ...
    def create_activity_log(self):
        ...
        log_label.setProperty("class", "activity-log-title")
    ...
    def _launch_app(self, key, name):
        ...
        if os.path.exists(app_path):
            try:
                ...
                self.log_message(f"[OK] Launched {name}")
            except Exception as e:
                self.log_message(f"[ERROR] Launch failed: {e}")
                QMessageBox.critical(self, "Launch Error", f"Failed to launch {name}:\n{e}")
        else:
            # Try alternative paths
            found = False
            for alt_path_rel in ALLOWED_APPS[key].get('alt_paths', []):
                alt_path = os.path.join(base_dir, alt_path_rel)
                if os.path.exists(alt_path):
                    try:
                        subprocess.Popen([sys.executable, alt_path], cwd=os.path.dirname(alt_path))
                        self.log_message(f"[OK] Launched {name} (alt path)")
                        found = True
                        break
                    except Exception as e:
                        self.log_message(f"[ERROR] Launch failed (alt): {e}")
            
            if not found:
                self.log_message(f"[ERROR] {name} not found at {app_path}")
