...
Source: "drivers\SQLServerExpress.exe"; DestDir: "{app}\drivers"; Flags: ignoreversion external; Tasks: installsqlserver
...
; NOTE: Don't use "Flags: ignoreversion" on "shared\*", "AutoDiag\*" and similar entries
; if you have files in /shared and /AutoDiag directories that should always be overwritten.
...
begin
  ...
  begin
    V := MsgBox('Python is required to run AutoDiag Pro but does not appear to be installed. Would you like to open the Python installation guide?', mbConfirmation, MB_YESNO or MB_DEFBUTTON2);
    if V = IDYES then
    begin
      Exec(ExpandConstant('https://www.python.org/downloads/'), '', '', SW_HIDE, ewWaitUntilTerminated, iResultCode);
      // Open the Python installation guide
      if FileExists(ExpandConstant('{app}\Documentation\Python_Installation_Guide.txt')) then
        Exec('notepad.exe', ExpandConstant('{app}\Documentation\Python_Installation_Guide.txt'), '', SW_HIDE, ewWaitUntilTerminated, iResultCode);
    end;
    Result := False;
    exit;
  end;
  ...
  begin
    V := MsgBox('Some required Python packages are missing. Would you like to install them now using pip?', mbConfirmation, MB_YESNO or MB_DEFBUTTON2);
    if V = IDYES then
    begin
      // Install required packages
      if not Exec('python', '-m pip install -r requirements.txt', '', SW_HIDE, ewWaitUntilTerminated, iResultCode) then
      begin
        MsgBox('Failed to install required packages. Please check the requirements.txt file and install packages manually.', mbError, MB_OK);
        Result := False;
        exit;
      end;
    end;
  ...
  begin
    // Check if bundled SQL Server installer is available
    if FileExists(ExpandConstant('{app}\drivers\SQLServerExpress.exe')) and WizardIsTaskSelected('installsqlserver') then
    begin
      V := MsgBox('SQL Server is required but not detected. Would you like to install the bundled SQL Server Express now?', mbConfirmation, MB_YESNO or MB_DEFBUTTON2);
      if V = IDYES then
      begin
        // Run the bundled installer
        if Exec(ExpandConstant('{app}\drivers\SQLServerExpress.exe'), '/QUIET /IACCEPTSQLSERVERLICENSETERMS', '', SW_HIDE, ewWaitUntilTerminated, iResultCode) then
        begin
          MsgBox('SQL Server Express installation completed. The setup will continue.', mbInformation, MB_OK);
        end else
        begin
      ...
      begin
        MsgBox('SQL Server is required for AutoDiag Pro to function properly. Please install SQL Server Express before continuing.', mbError, MB_OK);
        Result := False;
        exit;
      end;
    end else
    begin
      V := MsgBox('SQL Server is required for the user database but does not appear to be installed. Would you like to open the SQL Server Express installation page?', mbConfirmation, MB_YESNO or MB_DEFBUTTON2);
      if V = IDYES then
      begin
        ShellExec('open', 'https://www.microsoft.com/en-us/download/details.aspx?id=104781', '', '', SW_SHOWNORMAL, ewNoWait, iResultCode);
        MsgBox('Please download and install SQL Server Express, then run this installer again.', mbInformation, MB_OK);
        Result := False;
        exit;
      end else
      begin
  ...
  // Note: VCI driver installation is handled in CurStepChanged after wizard initialization
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        while True do
        begin
          if not Exec(ExpandConstant(sUnInstallString), '', '', SW_HIDE, ewWaitUntilTerminated, iResultCode) then
            break;
          Sleep(1000);
        end;
      end else
...
begin
  ...
  begin
    ...
    begin
      if MsgBox('Would you like to install the bundled VCI drivers? This will install common J2534 PassThru and ELM327 drivers.', mbConfirmation, MB_YESNO) = IDYES then
      begin
        // Try to run common driver installers
        if FileExists(ExpandConstant('{app}\drivers\setup.exe')) then
          Exec(ExpandConstant('{app}\drivers\setup.exe'), '', '', SW_HIDE, ewWaitUntilTerminated, iResultCode);
        if FileExists(ExpandConstant('{app}\drivers\install.exe')) then
          Exec(ExpandConstant('{app}\drivers\install.exe'), '', '', SW_HIDE, ewWaitUntilTerminated, iResultCode);
        if FileExists(ExpandConstant('{app}\drivers\driver.msi')) then
          Exec('msiexec.exe', ExpandConstant('/i "{app}\drivers\driver.msi" /quiet'), '', SW_HIDE, ewWaitUntilTerminated, iResultCode);
      ...
      begin
        ...
        if FileExists(ExpandConstant('{app}\Documentation\VCI_Installation_Guide.txt')) then
          Exec('notepad.exe', ExpandConstant('{app}\Documentation\VCI_Installation_Guide.txt'), '', SW_HIDE, ewWaitUntilTerminated, iResultCode);
      end;
    end;
  end;
end;